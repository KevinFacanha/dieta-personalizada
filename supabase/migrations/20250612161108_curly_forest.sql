...
-- Adiciona validações básicas para os campos (sem subqueries)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'points_non_negative'
  ) THEN
    ALTER TABLE user_points
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'total_points_earned_non_negative'
  ) THEN
    ALTER TABLE user_points
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'points_history_is_array'
  ) THEN
    ALTER TABLE user_points
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'redeemed_meals_is_array'
  ) THEN
    ALTER TABLE user_points
